...
export default class ServerResource extends InternalResource<IServerConfig> {
  ...
  register_config(config: IServerConfig) {
    this.config = config;
    for (const key of Object.keys(this.config.global_vars))
      this.server.set(key, this.config.global_vars[key]);
  ...
  parse_routes(router?: express.Router, _routes?: IServerRoute[]) {
    ...
    routes.forEach((route) => {
      ...
      if (
        !Object.keys(route).includes("routes") &&
        Object.keys(route).includes("method")
      ) {
        const logic_functions: ((req: Request, res: Response) => any)[] = [];
        for (const logic_name of Object.keys(this.logic)) {
          logic_functions.push(
            this.logic[logic_name].get_logic() as (
              req: Request,
              res: Response,
            ) => any,
          );
  ...
  run() {
    ...
    if (this.config != undefined) {
      const port: number =
        typeof this.config.port == "string"
          ? parseInt(this.config.port)
          : this.config.port;
      this.server.listen(port, "0.0.0.0", () => {
        console.log(
          `[DEBUG]: SERVER LISTENING ON PORT http://0.0.0.0:${this.config?.port}`,
        );
      });
    }
  }
